    ...
    try:
        ...
        if not results:
            results.append({
                ...
                "snippet": f"No instant answers found for '{query}'. Click to see full search results.",
        ...
        return [{
            ...
            "url": f"https://duckduckgo.com/?q={query.replace(' ', '+')}",
            "snippet": f"Search encountered an error: {str(e)}. Please try the direct search link.",
...
async def summarize_results(args: tuple) -> str:
    ...
    summary_parts.append("💡 **Summary**: These results provide comprehensive information about your search topic. Click the links to explore further.")
